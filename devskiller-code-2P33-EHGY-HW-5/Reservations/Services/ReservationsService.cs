namespace Reservations.Services
{
    ...
    public class ReservationsService : IReservationsService
    {
        ...
        public IEnumerable<ReservationItem> GetByDay(DateTime day, int hallNumber)
        {
            ...
            var result = _queryAllLectureHalls.Execute().FirstOrDefault(x => x.Number == hallNumber);
               if(result==null)
               {
                   throw new ArgumentException("Hall number does not exits");
               }

               reservations = _queryAll.Execute().Where(x=> x.From.Date==day.Date && x.Hall.Number == hallNumber).OrderBy(x=>x.From).ToList();
        ...
        public IEnumerable<HallFreeHoursStatisticsItem> GetHallsFreeHoursByDay(DateTime day)
        {
            ...
            var allLectureHall = _queryAllLectureHalls.Execute();
            var result  = new List<HallBusyHoursStatisticsItem>();
            var freeHours = new List<HallFreeHoursStatisticsItem>();
            foreach(var hall in allLectureHall)
            {
             var res = _queryAll.Execute().Where(x=>x.Hall.Number==hall.Number).GroupBy(x=>x.Hall.Number).Select(p=>new {grpField =p.Key,
                                                                      calculateValue=p.Where(x=>x.From.Date==day.Date).Sum(x => x.To.Subtract(x.From).TotalHours)}).ToList();
             foreach (var r in res)
             {
                 result.Add(new HallBusyHoursStatisticsItem() { HallNumber = r.grpField, BusyHoursNumber = int.Parse(r.calculateValue.ToString()) });
             }
            ...
            foreach(var hall in allLectureHall)
            {
                int hallnuber = hall.Number;
                int hours = 10-(result.FirstOrDefault(x=>x.HallNumber==hall.Number)==null?0:result.FirstOrDefault(x=>x.HallNumber==hall.Number).BusyHoursNumber);

                freeHours.Add(new HallFreeHoursStatisticsItem() { HallNumber = hallnuber, FreeHoursNumber = hours });
            }
            return freeHours;
        }
